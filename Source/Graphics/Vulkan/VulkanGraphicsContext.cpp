...
class VulkanGraphicsContext::MemoryTracker
{
public:
...
// static
const vk::DispatchLoaderDynamic& VulkanGraphicsContext::GetDynamicLoader()
{
   return dispatchLoaderDynamic;
}

// static
vk::DispatchLoaderDynamic VulkanGraphicsContext::dispatchLoaderDynamic;

VulkanGraphicsContext::VulkanGraphicsContext(Window& window)
   : GraphicsContextBase(window)
{
...
#  undef FIND_VULKAN_FUNCTION
   ...
   memoryCallbacks.pfnAllocate = &VulkanGraphicsContext::MemoryTracker::onAllocate;
   memoryCallbacks.pfnFree = &VulkanGraphicsContext::MemoryTracker::onFree;
...
void VulkanGraphicsContext::setFrameIndex(uint32_t index)
{
   GraphicsContextBase::setFrameIndex(index);

   ASSERT(delayedObjectDestroyer);
   delayedObjectDestroyer->onFrameIndexUpdate();
}

vk::DescriptorSetLayout VulkanGraphicsContext::getDescriptorSetLayout(const vk::DescriptorSetLayoutCreateInfo& createInfo) const
{
   ASSERT(layoutCache);
   return layoutCache->getLayout(createInfo);
}

void VulkanGraphicsContext::delayedDestroy(uint64_t handle, vk::ObjectType type, VmaAllocation allocation) const
{
   ASSERT(delayedObjectDestroyer);
   delayedObjectDestroyer->delayedDestroy(handle, type, allocation);
}

void VulkanGraphicsContext::delayedFree(uint64_t pool, uint64_t handle, vk::ObjectType type) const
{
   ASSERT(delayedObjectDestroyer);
