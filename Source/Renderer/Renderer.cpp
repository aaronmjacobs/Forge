...
void Renderer::render(vk::CommandBuffer commandBuffer, const Scene& scene)
{
   ...
   tonemapPass->render(commandBuffer, currentSwapchainTexture, hdrResolveTexture ? *hdrResolveTexture : *hdrColorTexture, bloomEnabled ? bloomPass->getOutputTexture() : nullptr, uiColorTexture.get(), renderSettings.tonemapSettings, context.getSwapchain().getSurfaceFormat().colorSpace);
